...
export const VisitsChart: React.FC = () => {
  ...
  const formatData = (analytics: AnalyticsItem[]): VisitData[] => {
    ...
    return days.map((day, index) => {
      ...
      const thisWeekVisits = analytics.filter(item => {
      ...
      const lastWeekVisits = analytics.filter(item => {
        const itemDate = new Date(item.lastVisited);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 bg-opacity-50 dark:bg-opacity-50 backdrop-filter backdrop-blur-lg rounded-xl p-6 shadow">
      ...
      <div className="flex items-center mt-1">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Total Visits Today: <span className="font-semibold text-gray-900 dark:text-gray-100">{todayVisits} visits</span>
        </p>
      </div>
    </div>
  );
